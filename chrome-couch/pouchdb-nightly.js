// pouchdb.nightly - 2014-01-15T16:26:17
  ...
  function readDocs() {
    ...
    src.allDocs({keys: ids, include_docs: true}, function(err, res) {
      ...
      res.rows.forEach(function(row, i) {
        ...
        if (parseInt(row.value.rev, 10) > 1) {
          return requests.enqueue(eachRev, [row.id, revs[i]]);
        }
        // if document has attachments, fetch it again single with attachments
        if (row.doc._attachments && Object.keys(row.doc._attachments).length) {
          return requests.enqueue(eachRev, [row.id, revs[i]]);
        }
        result.docs_read++;
